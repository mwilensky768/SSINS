...
def mwa_pointings(az,alt,tolerance=.01):
    #Order is Az, Alt
    pting_trans = {-3:(90,69.1655),
          -2:(90,76.2838),
          -1:(90,83.1912),
          0:(None,90),
           1:(270,83.1912),
...
def unpack_indices(pos_list,shape,init_check=True):
    #A recursive function which takes a list of 1-d indices and an array shape and determines what multidimensional indices it would have if the array was in the given array shape. init_check is reserved for the recursive use of this function
...
def cust_cmap(name):
    if name=='suncycle':
        ...
        cmap = np.array([ initial_map[:,0]**.8, initial_map[:,1]**2, initial_map[:,2]*.35, initial_map[:,0]]).transpose()
        cmap = mpl.colors.ListedColormap(cmap, name=name, N=cmap.shape[0])
          
    if name=='miami':
        initial_map = mpl.cm.twilight(np.arange(256*2))
        cmap = np.array([ initial_map[:,0]**.5, initial_map[:,1]**2, 1-initial_map[:,2], initial_map[:,0]]).transpose()
        cmap = mpl.colors.ListedColormap(cmap, name=name, N=cmap.shape[0])
    
    if name=='neonbow':
        rgb=color_maker(np.pi/2,[.75,-.75,0.25],[1,1,.5],[1,1,1],[1,1,1],[1,1,2]).rgb
        cmap = np.array(rgb).transpose()
        cmap = mpl.colors.ListedColormap(cmap, name=name, N=cmap.shape[0])
        
    if name=='smoothbow':
        rgb=color_maker(.4,[.65,0,0.35],f=[1,1,1],A=[1,1,.8],B=[1,1,1],p=[1,1,.5]).rgb
        cmap = np.array(rgb).transpose()
        cmap = mpl.colors.ListedColormap(cmap, name=name, N=cmap.shape[0])
...
class color_maker:
    ...
    def __init__(self,global_phi=0,phi=[0,0,0],f=[1,1,1],A=[1,1,1],B=[1,1,1],p=[1,1,1],name='default'):
        ...
        self.B = B
        self.p = p
        self.rgb = [self.A[0]*(1/2-self.B[0]*np.sin(self.f[0]*x-self.phi[0])/2)**self.p[0], 
                    self.A[1]*(1/2-self.B[1]*np.sin(self.f[1]*x-self.phi[1])/2)**self.p[1], 
                    self.A[2]*(1/2-self.B[2]*np.sin(self.f[2]*x-self.phi[2])/2)**self.p[2], 
                    np.ones(512)]
        self.name=name
    def rgb(self):
        return self.rgb
    ...
    '''def plot(self):
        plt.plot(self.rgb[0],color='red')
        plt.plot(self.rgb[1],color='green')
        plt.plot(self.rgb[2],color='blue')
    def cmap(self):
        new_map = np.array(self.rgb).transpose()
        cmap = new_map
        cmap = mpl.colors.ListedColormap(cmap, name=self, N=cmap.shape[0])
        return cmap'''
    ...
    global_phi = np.pi/2
    phi = [.75,-.75,0.25]
    phi = np.array(phi)*2*np.pi+global_phi
    f = [1,1,.5]
    A = [1,1,1]
    B = [1,1,1]
    p = [1,1,2]'''
...
def uvfits_reader(obs_id,uvfits_folder='',conjugate_baselines=False,split_autos=False,options={},use_ss_as_uvd=False,metafits_ant_check=True, experimental_ant_check=False,antenna_position_flags_csv=''):
    #Note: there are 3 ways to check for bad antennas. The preferred method is to use the metafits file. An alternate method is to use an antenna_position_flags_csv file for the closest available date, which flags based on antenna location. The experimental_ant_check method is unreliable and should not be used.
    obs_id=str(obs_id)
    fits_file = obs_id+'.uvfits'
    read_file = os.path.join(uvfits_folder, fits_file)
    print('reading ',read_file)
    if use_ss_as_uvd==False:
    ...
    if conjugate_baselines==True:
        print('conjugating bls')
        uvd.conjugate_bls(convention='v>0')

    if metafits_ant_check==True:
    ...
    if antenna_position_flags_csv!='' and metafits_ant_check==False:
        ...
        for ind, row in enumerate(antenna_position_flags):
            antenna_position_flags[ind] = [float(item) for item in row[:3]] + [ast.literal_eval(row[3])]
    ...
    if split_autos ==False:
        ...
        if experimental_ant_check==True and antenna_position_flags_csv=='':
            print('Warning: using experimental antenna checking. This method is unreliable and should not be used except for debuggging purposes.')
            cut_antennas = []
            baseline_power = np.mean(np.abs(data(uvd)),axis=(0,2,3))
            baseline_power = baseline_power/np.median(baseline_power)
            baseline_check = np.array(baseline_power>10) | np.array(baseline_power<.1)
            cut_antennas = list(np.unique(uvd.ant_1_array.reshape(uvd.Ntimes,uvd.Nbls)[0,:][baseline_check]))+list(np.unique(uvd.ant_2_array.reshape(uvd.Ntimes,uvd.Nbls)[0,:][baseline_check]))
            cut_antennas = np.unique(cut_antennas)
            if len(cut_antennas)>0:
                print('Experimental antenna checking identified the following antenna numbers as bad:',cut_antennas)
                keep_antennas = uvd.antenna_numbers
                keep_antennas = [ant for ant in keep_antennas if ant not in cut_antennas]
                uvd.select(antenna_nums=keep_antennas)
...
def shape_dict(shape_name,add_subTV=False):
    if shape_name=='MWA':
        with open(f"{SSINS_data.DATA_PATH}/MWA_EoR_Highband_shape_dict.yml", 'r') as shape_file:
...
def closest(lst, K, ineq=None):
    ...
    #ineq can be 'g' for greater than or equal to, 'l' for less than or equal to, or None to just go to the closest value

    lst = np.asarray(lst)
    if type(K)==type(1*u.Hz):
        K = K.value
    ...
    if lst[idx]<K and ineq=='g':
        if (sorted(lst)==lst).all()==False:
            raise ValueError('To use a ineq argument other than None the input array must be sorted smallest to largest')
        idx+=1  
        if idx>len(lst)-1:
            raise ValueError('Largest value of array = '+str(lst[-1])+ ', which is smaller than specified search value. Remove ineq=\'g\' or choose smaller value') 
        ...
        idx-=1
        if idx<0:
            raise ValueError('Smallest value of array = '+str(lst[0])+ ', which is larger than specified search value. Remove ineq=\'l\' or choose larger value') 
    ...
    for item in ranges:
        ...
        if type(item)==list or type(item)==tuple:
            if len(item)!=2:
                ...
                raise ValueError('Freq ranges must be ordered from smallest to largest, '+str(item[0])+' >= '+str(item[1]))
            ...
            if strict==True and warning_count_strict==0:
                print('Warning: \'strict\' keyword only affects frequency ranges, not individual frequencies')
...
def make_freq_mask(freqs,ranges,strict=True,return_list=False,shape_dict=None):
    ...
    for item in ranges:
        if type(item)==list:
            for subitem in item:
                if type(subitem)!=type(1*u.Hz):
                    print('Warning: please ensure the ranges arguments of make_freq_mask contains frequencies or pairs of \
frequencies and not indices')
                    break
        elif type(item)!=type(1*u.Hz):
            print('Warning: please ensure the ranges arguments of make_freq_mask contains frequencies or pairs of \
frequencies and not indices')
            break
...
def coarse_band_flagging( Nfreqs=384,coarse_band_count = 24, flag_centers=True,flag_edges=True,Ntimes=None,Npols=None, bad_edge_channel_fraction=1/8):
    ...
    #Edge size is the number of channels above and below each coarse band threshold that should be cut. Note it always will cut at least one
    ...
    for channel in range(Nfreqs):
        ...
        if flag_edges==True:
            ...
            if 0 in (channel+1+edge_channel_distances)%coarse_band_channel_size or 0 in (channel-edge_channel_distances)%coarse_band_channel_size :
                coarse_band_flags_ind.append(channel)

        #Flags centers of coarse bands
        if flag_centers==True:
    ...
    if type(Ntimes) == type(None) and type(Npols)== type(None):
        return coarse_band_flags
    elif type(Ntimes)!=type(Npols):
...
def recursive_search(folder,exclude_list = []):
    ...
    for item in os.listdir(folder):
        ...
        if len(item.split('.'))<2 and not item in exclude_list:
