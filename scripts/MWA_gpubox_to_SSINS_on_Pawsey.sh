...
if [ ! -e ${outdir}/${obs}_SSINS_data.h5 ]; then
  ...
  command="python3 /group/mwa/software/SSINS/csv_script/Scripts/MWA_vis_to_SSINS.py -o $obs -d ${outdir} -f $filelist $metafile"
  srun --export=all env - $(which singularity) exec -B /astro -B /group -B /group/mwaeor/mwilensky $SINGULARITY_CACHEDIR/python_ssins-2020-02-13.sif /bin/bash -c "$command"
